...
from PFE import db, message_handler
from PFE.forms import CreateRoomForm, JoinRoomForm
from PFE.models import Client, Room
...
async def index():
    ...
    if create_room_form.validate_on_submit():
        ...
        try:
            ...
            await flash('The room name "{}" is already exist'.format(name))
...
async def room(room_id):
    ...
    if client:
        ...
        return await render_template(
            'chatroom.html', title='PFE | {}'.format(room.name))
    ...
    return (
        (await render_template('join-room.html', title='PFE | Join a room',
                               form=form, room=room)),
        status_code)
...
async def public():
    ...
    return await render_template(
        'public.html', title='PFE Video Chat | Public Rooms',
